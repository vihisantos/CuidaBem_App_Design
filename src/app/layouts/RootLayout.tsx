          ...
          {navItems.map((item) => {
            ...
            return (
              <Link
                key={item.path}
                to={item.path}
                className={`flex flex-col items-center justify-center flex-1 h-full transition-colors relative group ${isActive ? "text-primary" : "text-neutral-400 hover:text-neutral-600"
                  }`}
              >
                {isActive && (
                  <span className="absolute -top-[1px] w-12 h-[3px] bg-primary rounded-b-full shadow-[0_2px_8px_rgba(45,156,219,0.5)]" />
                )}
