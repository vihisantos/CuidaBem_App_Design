        ...
        <div className="max-w-4xl mx-auto px-4 sm:px-6 py-4 space-y-4">
          <div className="relative">
            ...
            <input
              ...
              className="w-full bg-white dark:bg-slate-900 border border-border rounded-2xl pl-12 pr-4 py-3 text-foreground placeholder:text-muted-foreground/60 shadow-sm focus:ring-4 focus:ring-primary/10 focus:border-primary/50 transition-all outline-none font-medium"
              value={searchTerm}
          ...
          <div className="flex gap-3 overflow-x-auto pb-2 scrollbar-hide md:hidden">
            ...
            {categories.map(cat => (
              <button key={cat} className="whitespace-nowrap px-4 py-2 bg-white dark:bg-slate-900 border border-border rounded-xl text-sm font-medium text-muted-foreground shrink-0">
                {cat}
              </button>
            ))}
          </div>
          ...
          <div className="hidden md:flex gap-3">
            ...
            {categories.map(cat => (
              <button key={cat} onClick={() => toggleFilter(cat)} className={`whitespace-nowrap px-4 py-2 rounded-xl text-sm font-medium transition-colors ${selectedFilters.includes(cat) ? "bg-primary text-white" : "bg-white dark:bg-slate-900 border border-border hover:bg-muted"}`}>
                {cat}
              </button>
            ))}
          </div>
      ...
      <div className="max-w-4xl mx-auto px-4 sm:px-6 py-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
          {filteredCaregivers.map((caregiver, i) => (
            ...
            >
              <Link to={`/app/perfil/${caregiver.id}`}>
                <Card className="p-4 border-none shadow-sm hover:shadow-lg hover:-translate-y-1 transition-all duration-300 bg-white dark:bg-slate-900 group">
